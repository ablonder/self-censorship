...
public class Observer implements ObserverInterface {
	...
	public void upDateHistogramChart(int index, double [] data, long upDateInterval ) {
		if(!state.gui.chartTypeH) {
			System.out.println("Histogram chart is null!");
			return;//nothing to do
		}
		state.gui.chartHistogram.updateSeries(0, data);
		state.gui.chartHistogram.updateChartWithin(index, upDateInterval);  // update within one second (1000 milliseconds))
	}
