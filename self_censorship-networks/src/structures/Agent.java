...
public class Agent extends RandomWalkerContinuousAbstract {
	...
	public void play(Environment state){
		...
		//if pro-government policy, signal regardless of sensitivity
		//TODO 1. use belief as an indicator for speaking up or not;2.make sure the baseline is the same
		if(this.belief > 0.5) {
		...
		//dissidents speak up based on their belief and political sensitivity
		else {
			if(state.random.nextBoolean(1 - 2 * this.sensitivity * this.belief)) {
				this.signal = -1;
			}
			else {
				this.signal = 0;
		...
		//TODO 1. if the math is okay; 2.updating system (find some literature)
